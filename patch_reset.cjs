...
// Add password validation to handleImportJSON
const oldImportJSON = `  const handleImportJSON = async (e: React.ChangeEvent<HTMLInputElement>) => {
...
const newImportJSON = `  const handleImportJSON = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
...
code = code.replace(oldImportJSON, newImportJSON);
