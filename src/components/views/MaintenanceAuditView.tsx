    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 shadow-sm mb-6 max-w-6xl mx-auto space-y-6">
      ...
      {activeTab === 'traceability' && (
        <div className="space-y-6">
          ...
          {/* Go-Live Opening Stock & System Setup Audit Trail Block */}
          {logs.some(l => l.jobId === 'SYSTEM-SETUP' || l.stage === 'System Setup') && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="text-amber-600 font-bold">⚡</span>
                  <h4 className="text-xs font-black text-amber-900 uppercase tracking-wide m-0">
                    Go-Live Opening Stock & Inward Audit Logs
                  </h4>
                </div>
                <span className="text-[10px] text-amber-700 font-black bg-amber-100 px-2.5 py-0.5 rounded-full">
              ...
              <div className="overflow-x-auto border border-amber-200 rounded-lg bg-white">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-amber-50 border-b border-amber-200 text-amber-900 font-extrabold uppercase text-[10px]">
                      <th className="p-2.5">Timestamp</th>
                      <th className="p-2.5">User</th>
                      <th className="p-2.5">Inward Action Summary</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-amber-100 font-medium text-slate-800 text-[11px]">
                    {logs
                      .filter(l => l.jobId === 'SYSTEM-SETUP' || l.stage === 'System Setup')
                      .slice()
                      .reverse()
                      .map((l, idx) => (
                        <tr key={idx} className="hover:bg-amber-50/30 transition">
                          <td className="p-2.5 whitespace-nowrap font-semibold text-slate-500">{l.timestamp}</td>
                          <td className="p-2.5 font-bold text-amber-950 whitespace-nowrap">{l.user || 'ADMIN'}</td>
                          <td className="p-2.5 text-slate-700 font-medium">{l.action}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
