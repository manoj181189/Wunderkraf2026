...
import { ArrowLeft, Cog, Play, Pause, Square, Zap, Undo2, XCircle, Check, Layers, AlertCircle } from 'lucide-react';
...
export const FormingView: React.FC<FormingViewProps> = ({
  ...
  const handleConfirmForwardToQC = () => {
    ...
    if (qty > curIssued) {
      alert(`Cannot forward more than currently issued batch qty (${curIssued})!`);
      return;
    }
    ...
    const newLog = {
      ...
      machine: selectedMachine,
      shift: batch.shift,
      action: `⚡ Partial Forward: ${qty} Formed Crates forwarded to QC Desk (Batch #${batch.batchId})`,
      worker: batch.worker,
      user: 'form_user',
      rawDate: new Date().toISOString().split('T')[0],
      timestamp: new Date().toLocaleString()
    ...
    setForwardQtyInput('');
    alert(`✅ Forwarded ${qty} Formed Crates to QC Inspection Desk! Machine remains RUNNING.`);
  };
  ...
  const handleFinish = () => {
    ...
    const newLog = {
      ...
      machine: selectedMachine,
      shift: batch.shift,
      action: `⏹️ Finished Forming Batch ${batch.batchId} (${cratesDone} Crates, Defect Pieces: ${scrapPcsVal})`,
      worker: batch.worker,
      user: 'form_user',
    ...
    setScrapPcs('0');
    setSelectedActiveBatchId('');
    alert(`✅ Forming Run Finished! Added ${cratesDone} Formed Crates to inventory.`);
  };
      ...
      <div className="border-t border-slate-200 pt-4">
        ...
        <form onSubmit={handleStartOrTopup} className="space-y-4">
          ...
          >
            ...
            <span>Start / Issue Crates to {selectedMachine}</span>
          </button>
        </form>
      </div>
